...
try{
 ...
 const api=await import(pathToFileURL(output)),calls=[],actorId='provider-operator',controller=new AbortController();let session={user:{id:actorId}};
 ...
 globalThis.providerRpc=async(resource,action,payload,signal)=>{calls.push({resource,action,payload,signal});return resource==='service-requests'?{request:{providerId:payload.providerId}}:action==='list'?{providers:[provider],canManage:true}:{provider};};
 ...
 console.log('Passed: provider RPC data, CPF/CNPJ presentation, CNPJ lookup mapping/failure, server-resolved request provider and account cancellation.');
