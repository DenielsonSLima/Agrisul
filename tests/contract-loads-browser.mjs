...
export async function verifyLoadsBrowser({cdpUrl,baseUrl,session,projectRef,contractId}){
 ...
 try{
  ...
  await evaluate(`${row}.querySelector('button[title="Editar carregamento"]').click()`);await waitFor("!!document.querySelector('.contract-load-form')",'edit load');await fill('input[name=volume]','1000');await waitFor("document.querySelector('.contract-load-excess-warning')?.textContent.includes('incluído normalmente no faturamento')",'excess warning');await click('Salvar alterações');await waitFor("!document.querySelector('.contract-load-form')",'edited excess load');await waitFor(volume('1.100,00 t'),'excess load KPIs');
  await evaluate(`${row}.querySelector('button[title="Excluir carregamento"]').click()`);await waitFor("!!document.querySelector('[role=alertdialog]')",'delete confirmation');await click('Cancelar',"document.querySelector('[role=alertdialog]')");assert.equal(await evaluate(volume('1.100,00 t')),true);
  ...
  console.log('PASS: browser filters and KPIs, grouping, contextual PDF, dependent origins, create/edit/delete, excess warning, empty state and responsive layout.');
