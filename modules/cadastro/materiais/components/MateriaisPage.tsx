...
function MaterialList({materials,filtered,groups,pagination,categories,loading,error,categoriesError,search,categoryFilter,onSearch,onCategory,onPage,onClearFilters,onReload,onReloadCategories,onCreate,onOpen,onEdit,onDelete,onQuotes}:MaterialListProps){
 return <>
  ...
  {loading?<div className="client-loading" role="status"><Loader2 size={20} className="animate-spin"/>Carregando materiais…</div>:error?<div className="company-empty" role="alert"><h3>Não foi possível carregar</h3><p>{error}</p><button type="button" className="btn" onClick={()=>void onReload()}><RefreshCw size={16}/>Tentar novamente</button></div>:!materials.length?<div className="company-empty"><span className="company-empty-icon"><ClipboardList size={25}/></span><h3>Cadastre seu primeiro material</h3><p>O produto terá uma única foto e poderá receber várias referências equivalentes.</p><button type="button" className="btn company-primary" onClick={onCreate}><Plus size={16}/>Cadastrar material</button></div>:!filtered.length?<div className="company-empty"><span className="company-empty-icon"><Search size={25}/></span><h3>Nenhum material encontrado</h3><p>Ajuste a busca ou o filtro de categoria.</p><button type="button" className="btn" onClick={onClearFilters}>Limpar filtros</button></div>:<div className="material-card-groups">{groups.map(group=>{const headingId=`material-group-${group.id}`;return <section className="material-card-group" key={group.id} aria-labelledby={headingId}><header className="material-card-group-heading"><h3 id={headingId}><FolderTree size={15}/>{group.name}</h3><small>{group.items.length} materia{group.items.length===1?'l':'is'} nesta página</small></header><div className="materials-card-grid" role="list">{group.items.map(item=><article className="material-catalog-card" key={item.id} role="link" tabIndex={0} aria-label={`Abrir material ${item.name}`} onClick={()=>onOpen(item.id)} onKeyDown={event=>{if(event.target===event.currentTarget&&event.key==='Enter'){event.preventDefault();onOpen(item.id);}}}><div className="material-catalog-photo">{item.imageUrl?<img src={item.imageUrl} alt="" loading="lazy"/>:<ClipboardList size={30}/>}<span className="material-code-chip">{item.internalCode||'Sem código'}</span></div><div className="material-catalog-content"><div className="material-catalog-title"><h4>{item.name}</h4><div className="material-card-actions"><button type="button" className="icon-btn" aria-label={`Editar ${item.name}`} onClick={event=>{event.stopPropagation();onEdit(item);}}><Pencil size={15}/></button><button type="button" className="icon-btn" aria-label={`Excluir ${item.name}`} onClick={event=>{event.stopPropagation();onDelete(item);}}><Trash2 size={15}/></button></div></div><p>{item.application||'Sem descrição informada'}</p><footer><span>{item.unit}</span><span><Tag size={13}/>{item.references.length} referência{item.references.length===1?'':'s'}</span><ChevronRight size={17} aria-hidden="true"/></footer></div></article>)}</div></section>;})}</div>}
