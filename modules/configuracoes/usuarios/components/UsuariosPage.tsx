...
import {Loader2,MailPlus,Pencil,Power,PowerOff,RefreshCw,ShieldCheck,Users} from 'lucide-react';
...
export function UsuariosPage(){
  ...
  const changeStatus=async(member:BillingUser)=>{
    ...
    if(!enabling){const accepted=await confirm({title:pending?`Cancelar convite de ${member.email}?`:`Inativar ${member.name||member.email}?`,description:pending?'O convite pendente será cancelado e não poderá mais ser aceito.':'O usuário perderá imediatamente o acesso aos dados deste espaço de trabalho. O cadastro poderá ser reativado depois.',confirmLabel:pending?'Cancelar convite':'Inativar usuário',tone:'destructive'});if(!accepted)return;}
    setActionId(member.id);setActionError('');
    try{await users.setEnabled(member.id,enabling);notifications.updated(enabling?'O usuário foi reativado.':pending?'O convite foi cancelado.':'O usuário foi inativado.');}
