...
import {fetchUsers,inviteUser,setUserEnabled,updateUserAccess} from '../services/usersApi';
...
export function useUsers(){
  ...
  return {users:query.data??[],loading:!ready||!!user&&query.isPending,error:query.error?.message??'',authRequired:ready&&!user,saving:inviteMutation.isPending||updateMutation.isPending||statusMutation.isPending,reload:()=>query.refetch(),invite:(input:UserInviteInput)=>inviteMutation.mutateAsync(input),updateAccess:(id:string,accessProfileId:string)=>updateMutation.mutateAsync({id,accessProfileId}),setEnabled:(id:string,enabled:boolean)=>statusMutation.mutateAsync({id,enabled})};
}
