...
function ReceiptComposition({selected,total,month}:{selected:ContractFinancialMetrics;total:ContractFinancialMetrics;month:string}){
 ...
 return <section className="finance-chart-panel finance-composition" aria-label="Composição dos recebimentos">
  ...
  <div className="finance-donut" role="img" aria-label={`Total recebido líquido: ${money(metrics.receivedAmount)}. Adiantamentos: ${money(metrics.advanceAmount)}. Recebimentos: ${money(metrics.receiptAmount)}. Estornos: ${money(metrics.refundedAmount)}.`}>
  ...
  <dl className="finance-composition-legend"><div><dt><i className="advance"/>Adiantamentos</dt><dd>{money(metrics.advanceAmount)}</dd></div><div><dt><i className="receipt"/>Recebimentos</dt><dd>{money(metrics.receiptAmount)}</dd></div><div><dt><i className="refund"/>Estornos</dt><dd>− {money(metrics.refundedAmount)}</dd></div></dl>
