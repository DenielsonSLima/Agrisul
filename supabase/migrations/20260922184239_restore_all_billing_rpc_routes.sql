-- Quotations must extend the public RPC without removing routes installed by
-- later operational modules. Replacing these two functions is idempotent and
-- also repairs databases where the previous quotation migrations already ran.
...
BEGIN
 ...
 v_permission=CASE
  ...
  WHEN p_resource IN ('clients','atr','farms','plots','contract-types','cultures','cultural-practices','materials','quotations') THEN 'registrations.'||CASE WHEN p_action IN ('get','list') THEN 'read' ELSE 'write' END
...
COMMENT ON FUNCTION public.billing_rpc(text,text,jsonb) IS 'Authenticated workspace RPC. Quotations extend, without replacing, the routes for providers, requests, document templates, home and existing operational modules.';
