-- One database snapshot supplies the filtered list, counters and financial KPIs.
-- Reuse the contract's monthly financial rules, including cent rounding and
-- advances. Credit on one contract never pays the balance of another contract.
CREATE FUNCTION billing_private.contracts_list_summary(p_payload jsonb)
...
-- Preserve all existing detail, load and finance dispatch behavior.
ALTER FUNCTION billing_private.contracts_company_dispatch(text,jsonb) RENAME TO contracts_before_list_summary_dispatch;
REVOKE ALL ON FUNCTION billing_private.contracts_before_list_summary_dispatch(text,jsonb) FROM PUBLIC,anon,authenticated;
...
BEGIN
 ...
 IF p_action='list' AND coalesce(p_payload->>'view','')='' THEN RETURN billing_private.contracts_list_summary(p_payload); END IF;
 RETURN billing_private.contracts_before_list_summary_dispatch(p_action,p_payload);
END $$;
